...
var TemplatedTaskCreateDialog = React.createClass({
  ...
  create:function(){
    if(this.state.Name!="" && this.state.ConnectionString!=""&& this.state.Pack!=""&& this.state.DatabaseName!=""&& this.state.Repository!=""&& this.state.Machine!=""){
        Actions.createTemplatedTask(this.state.Name, this.state.ConnectionString,this.state.Pack,this.state.DatabaseName, this.props.EnviromentId,this.state.Repository,this.state.Machine, this.props.ApplicationId).then(x => {
